...
def mean_shift(out_file,if_single_img=True):
    ...
    # 使用 Mean Shift 聚类算法进行聚类
    mean_shift = MeanShift(bandwidth=1)  # MeanShift(bandwidth=100) 根据需要调整 bandwidth 参数
    mean_shift.fit(masked_pixels)
    end_time = time.time()  # 记录结束时间
    elapsed_time_ms = (end_time - start_time) * 1000  # 计算消耗的时间（毫秒）
    print("消耗的时间（毫秒）mean-shift:", elapsed_time_ms)
    # 获取聚类后的中心点
    ...
    opening = cv2.morphologyEx(result_image, cv2.MORPH_OPEN, kernel=np.ones((5, 5), np.uint8))
